...
object Day4 {
 ...
 def solution(input:List[String]): (Int,Int) ={
  ...
  def getLastWinningBoard(extractionNumbers: Seq[Int], boards: MarkBoards, winnerBoardNumber: Int): Winner ={

    val boardWinner = extractionWinner(extractionNumbers, boards, Seq.empty, -1)
    val boardsWithoutWinner = MarkBoards(boards.board.filter{ boardMatrix => boardMatrix._2 != boardWinner.winnerBoardNumber.get})
  ...
  @tailrec
  def extractionWinner(extractionNumbers:Seq[Int], boards: MarkBoards, winningBoard: Seq[Int], lastExtractedNumber: Int ): Winner ={
      winningBoard match {
        case Nil => {
                      val markedBoards: MarkBoards = mark(extractionNumbers.head, boards)
  ...
  def getWinningBoard(boards: MarkBoards): Seq[Int] = // checking only that the line is marked and not the column as well hence why I have the wrong answer for part 2.
    boards.board.collect{
      boardMatrix => boardMatrix._1
        .map { line => line.map(value => value._2) }
        .flatMap { value => if (value.sum == 5) Some(boardMatrix._2) else None }
    }.flatten


  def mark(extractedNumber: Int, boards: MarkBoards): MarkBoards = {
      MarkBoards(boards.board.map{ boardMatrix =>
  ...
  def getBoards(input: List[String]): MarkBoards = {
